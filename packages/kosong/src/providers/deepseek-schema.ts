/**
 * DeepSeek's Anthropic-compatible endpoint requires tool parameter schemas to
 * have an object root. Flatten only root unions made entirely of object
 * branches; preserve every other JSON Schema shape unchanged.
 */
export function normalizeDeepSeekToolSchema(
  schema: Record<string, unknown>,
): Record<string, unknown> {
  return flattenRootObjectUnion(schema) ?? schema;
}
