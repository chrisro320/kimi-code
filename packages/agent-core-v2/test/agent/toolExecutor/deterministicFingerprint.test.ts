...
import { DeterministicFailureFingerprint } from '#/agent/toolExecutor/deterministicFingerprint';
...
describe('DeterministicFailureFingerprint', () => {
  ...
  it('produces the same fingerprint for semantically-equal args (canonical key order)', () => {
    const fingerprint = new DeterministicFailureFingerprint();
    fingerprint.recordIfDeterministic('Read', { path: '/missing.txt' }, enoentResult('/missing.txt'));

    // Object key order must not change the fingerprint (canonicalTelemetryArgs).
    const reordered = JSON.parse('{"path":"/missing.txt"}') as unknown;
    expect(fingerprint.checkFingerprint('Read', reordered)).not.toBeNull();
  });
  ...
  it('unblocks a Read ENOENT after a successful Write creates the file (c58df026b)', () => {
    const fingerprint = new DeterministicFailureFingerprint();
    const args = { path: '/missing.txt' };
    fingerprint.recordIfDeterministic('Read', args, enoentResult('/missing.txt'));
