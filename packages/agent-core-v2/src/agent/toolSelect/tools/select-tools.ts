 ...
 *
 * Registers the built-in tool that lets the model load deferred schemas
 * (MCP + selected low-frequency builtins) named in loadable-tools
 * announcements. Delegates loading to `IAgentToolSelectService`; offered by
 * the shaped tool view only while the disclosure gate is open.
 */
