 ...
 * tool).
 *
 * The built-in tool that lets the model load deferred schemas — MCP, deferred
 * user tools, and the selected low-frequency builtins — named in loadable-tools
 * announcements. Delegates loading to `IAgentToolSelectService` (`toolSelect`
 * domain); offered by the shaped tool view only while the disclosure gate is
 * open.
 *
