/**
 * `dispatch` domain — workspace-relative dispatch-scope normalization shared
 * by the Agent/AgentSwarm tool-input validation and scope reservation. A
 ...
 *
 * Ported from v1 `agent/dispatch/scope.ts` — behavior must stay equivalent
 * (this module is the B6 worktree-isolation foundation).
 *
 * Ordering contract: any sorting of scope paths is by UTF-16 code unit
 * (default `Array.prototype.sort()`), never `localeCompare` — collation
 * disagrees with code-unit order on case and punctuation (`w-x.txt` vs
 * `w_x.txt`) and broke v1's manifest integrity check (`fbc784ec0`).
 */
