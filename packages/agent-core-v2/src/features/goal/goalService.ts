 ...
 *
 * Owns the main-agent goal lifecycle; persists the goal in the replayable
 * `goal` state key (`GoalState | null`) through the `goal.create` /
 * `goal.update` / `goal.clear` events (`dispatcher.dispatch`), reads it
 * through `agentState.get(goalKey)`,
