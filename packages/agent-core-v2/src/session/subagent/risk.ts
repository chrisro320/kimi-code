 ...
 * (e.g. `SessionSwarmService.run`) adapt their own task/profile resolution
 * into this shape so the risk checks stay pure and independently testable,
 * with no dependency on `Agent`/`AgentProfile`/DI services.
 *
 * Ported from v1 `session/subagent-risk.ts` — behavior must stay equivalent.
 */
 ...
 * (`agent/dispatch/controller.ts` in v1) — an already-calibrated concurrency
 * limit for the same kind of editing-capable dispatch — rather than an
 * arbitrary number.
 */
 ...
 *
 * Uses a direct `git status` spawn rather than a DI git service: the check
 * must stay a pure function of `(items, workspaceDir)` so it remains
 * testable without a session container, same as v1.
 *
