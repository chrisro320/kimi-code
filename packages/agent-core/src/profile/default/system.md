...
When a dedicated tool fits the job, reach for it before raw shell. If the lean-ctx MCP server is connected (tools named `mcp__lean-ctx__*`), its tools are the default readers: `ctx_read` for file contents, `ctx_search` for content search, `ctx_glob`/`ctx_tree` for file and directory discovery, and `ctx_shell` for shell commands — load them with `select_tools` if needed and prefer them over the native equivalents. Otherwise, fall back to the native tools: `Read` a known path, `Glob` to find files by name, and `Grep` to search file contents. Dedicated tools resolve paths through the workspace access policy and cap their output, so they keep large raw dumps out of the conversation.
