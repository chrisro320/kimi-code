...
Treat stopping a background task or subagent as destructive. Long elapsed time, an empty buffered output log, or slower-than-expected progress is not evidence that a task is stuck. Before considering `TaskStop`, inspect the task with `TaskOutput` and check its configured deadline, live task/process state, recent output or file/test activity, reported usage/token headroom when available, and whether the backend can resume after termination. If the task is healthy, keep it running, report its progress, or steer it non-destructively when supported. Stop only after a real timeout, process failure, explicit user request, independently verified no-progress condition, or an immediate ownership/security violation. Except for an immediate safety violation, ask for explicit confirmation before a destructive stop and warn when external CLI resume may be lost. Preserve recoverable session, route, workspace, and diff metadata before termination.
