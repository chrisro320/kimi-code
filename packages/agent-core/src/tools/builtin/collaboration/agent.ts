...
function formatBackgroundAgentResult(
  ...
  if (handle.resumable === true || !isExternalSubagentId(handle.agentId)) {
    lines.push(
      `resume_hint: To continue or recover this same subagent later, call Agent(resume="${handle.agentId}", prompt="..."). The parameter is agent_id ("${handle.agentId}"), NOT task_id ("${taskId}") or source_id from a later <notification>. Recovery cases: a later <notification type="task.lost" | "task.failed" | "task.killed"> for this subagent — its configured session resume will pick it up when supported.`,
    );
  }
  return lines.join('\n');
}
...
function formatForegroundAgentFailure(
  ...
  if (timedOut && handle.resumable !== false && !isExternalSubagentId(handle.agentId)) {
    lines.push(
      `resume_hint: Continue with Agent(resume="${handle.agentId}", prompt="continue"). Use agent_id only; do not set subagent_type. In-process agents retain prior context; configured external agents resume when their backend session still exists.`,
    );
  }
  return lines.join('\n');
}
