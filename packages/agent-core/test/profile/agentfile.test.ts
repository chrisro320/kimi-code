...
describe('SessionAgentProfileCatalog', () => {
  ...
  it('extends the default delegation set with file-defined agents', async () => {
    ...
    expect(Object.keys(delegatable)).toContain('file-reviewer');
    expect(Object.keys(delegatable)).toContain('coder');
    // The builtin profile objects are shared constants; the extension must
    // not leak into them.
    expect(Object.keys(DEFAULT_AGENT_PROFILES['agent']!.subagents ?? {})).not.toContain('file-reviewer');
  });
  ...
  it('renders a file profile prompt through the builtin base prompt', async () => {
    ...
    const c = catalog({ workDir, brandHomeDir: brandHome, osHomeDir: osHome });
    await c.ready;
    const rendered = c.get('file-reviewer')!.systemPrompt(promptContext);
  ...
  it('backs ${base_prompt} with the SYSTEM.md override when present', async () => {
    ...
    expect(c.get('file-reviewer')!.systemPrompt(promptContext)).toBe('[Org default on Linux.]');
  });
...
describe('Session agentfile wiring', () => {
  ...
  it('binds a file-defined main profile selected by profileName', async () => {
    ...
    await writeFile(
      join(workDir, '.kimi-code', 'agents', 'file-reviewer.md'),
    ...
    const session = new Session({
      ...
      agents: { userHomeDir: osHome, profileName: 'file-reviewer' },
    });
    try {
      const main = await session.createMain();
      expect(main.config.profileName).toBe('file-reviewer');
      ...
      expect(Object.keys(session.agentCatalog.delegatableSubagents('agent'))).toContain('file-reviewer');
    } finally {
      await session.close();
    }
  });
