...
4. **`src/tui-alt-screen.ts` — no ED2 on full redraw**: the `fullRedraw` branch of `doRender` emits only the image-cleanup sequence, never `\x1b[2J`. The row loop below it rewrites every row of the viewport with its own `\x1b[2K`, so the clear cannot change the resulting frame — but on a terminal that ignores DECSET 2026 (synchronized output) the clear presents on its own and the viewport flashes empty before the repaint lands. This was reproducible in Orca (`TERM=xterm-256color`, `TERM_PROGRAM=Orca`), where switching panes resizes the terminal and therefore forces a full redraw. Note that `TerminalCapabilities.syncEnabled` still has no consumer: the 2026 wrapper is emitted unconditionally, and a frame must be correct without relying on it. Guarding tests: the "full redraw" group in `test/tui-alt-screen.test.ts`.
