...
- **Terminal / 视图分屏**：**先做 terminal 普通 tab**（与 chat/tasks/todo 同级，xterm + WS 数据通道跑通）；**完全体 = 视图分屏（tab 维度，像 VSCode 编辑器组）**——会话区任意横/竖劈成多个视图组，每组独立持有 chat/tasks/todo/files/terminal **之一**，可拖拽视图在组间移动、调比例、布局持久化；窄屏退化单组整屏切。**注意：分屏是视图/tab 维度，不是在 terminal 标签内部分屏**；terminal 只是可放进视图组的视图之一。
