...
- 为子 Skill 名称加上父级前缀，并在 TUI 中将子 Skill 作为点式斜杠命令暴露出来。

### 修复

- 允许将 `--auto`、`--yolo` 和 `--plan` 与 `--session` 或 `--continue` 组合使用，并将请求的模式应用到恢复后的会话。
- 在恢复已处于 Plan 模式的会话时跳过重复进入 Plan 模式（此前会报错 `Already in plan mode`），并在通过 `/sessions` 选择器切换会话时停止重复应用 `--auto`/`--yolo`/`--plan` 启动参数。
- 遇到无效的 config.toml 分区时改为告警并丢弃，而不是启动失败。
- 修复 iTerm2 中无休止桌面通知的问题，仅向支持终端进度序列的终端发送相关序列。
- 修复恢复会话时已完成和已取消压缩记录显示不正确的问题。
- 在启动刷新期间同步自定义 registry provider 的新增、移除和轮换后的 registry key。
