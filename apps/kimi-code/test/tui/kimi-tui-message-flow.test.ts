...
async function makeDriver(
  ...
  await driver.init();
  // The TUI always starts session-less now (lazy startup). Tests that do not
...
describe('KimiTUI message flow', () => {
  ...
  it('scans inline skill tokens in a plain prompt', async () => {
    ...
    await vi.waitFor(() => {
      expect(session.promptWithSkills).toHaveBeenCalledWith('please /skill:review this', [
        { name: 'review' },
      ]);
    });
    expect(session.prompt).not.toHaveBeenCalled();
  ...
  it('hydrates lazy config defaults on a sessionless /reload (v2 engine)', async () => {
    ...
    const startupInput: KimiTUIStartupInput = {
      ...makeStartupInput(),
      cliOptions: { ...makeStartupInput().cliOptions, model: undefined },
    };
...
`,
  ...
  it('does not create a session for an unknown workspace-file command', async () => {
    ...
    driver.handleUserInput('/does-not-exist .trellis/tasks/target/handoff.json');

    await vi.waitFor(() => {
      expect(stripSgr(renderTranscript(driver))).toContain('Invalid slash command: /does-not-exist');
    });
  ...
  it('hydrates MCP server status after subscribing to session events', async () => {
    ...
    const { driver } = await makeDriver(session);
    // The lazy session creation already subscribed; re-run the subscription
    // under test to verify its subscribe-then-snapshot ordering.
  ...
  it('drains a queued image message with its media parts', async () => {
    ...
    // Queued parts are replayed verbatim.
    expect(session.prompt).toHaveBeenCalledWith(expected, { promptId: expect.any(String) });
  });
