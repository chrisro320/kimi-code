...
export class KimiTUI {
  ...
  suspendTerminalUi(): void {
    ...
    this.state.ui.stop({ preserveScreen: this.state.ui.mode === 'fullscreen' ? true : undefined });
  }
  ...
  async syncRuntimeState(session: Session = this.requireSession()): Promise<void> {
    ...
    this.setAppState({
      ...
      leanModeActive: status.leanMode === true,
  ...
  private unloadCurrentSession(reason: string): Session | undefined {
    ...
    this.setAppState({ goal: null, acp: undefined });
    return previous;
  }
