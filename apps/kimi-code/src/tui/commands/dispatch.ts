...
function dispatchInlineSkillCombo(host: SlashCommandHost, text: string): boolean {
  ...
  if (intent.kind !== 'skill' && intent.kind !== 'message' && intent.kind !== 'invalid') {
    return false;
  }
  ...
  // activation. The same holds for 'invalid': a leading skill token followed
  // by a newline makes the parsed name (`skill:a\n…`) unknown, but the input
  // is still a bundle.
  ...
  // A genuine single-skill command keeps its own path — skill tokens inside
  // its args are plain text, not activations.
  if (intent.kind === 'skill') return false;
  // An unrecognized leading slash token still gets scanned for inline skills
  // like any other plain prompt; only when no skill token is present does the
  // input fall through to the regular slash path (unknown-command error, or a
  // specially handled command like /dance).
  const activations = extractInlineSkillActivations(text, host.skillCommandMap);
...
async function handleBuiltInSlashCommand(
  ...
  switch (name) {
    ...
    case 'swarm':
      ...
      return;
    case 'dispatch':
      await handleDispatchCommand(host, args);
      return;
    case 'compact':
